...
model_path = os.path.join(os.path.dirname(__file__), "newmodel/license-plate-finetune-v1n.pt")
...
def _cleanup_broken_paddle_archive():
    ...
    tar_path = os.path.join(rec_dir, "en_PP-OCRv4_rec_infer.tar")
    if os.path.exists(tar_path):
        try:
            with tarfile.open(tar_path, "r") as tf:
                # iterate to force a full read of members
                _ = tf.getmembers()
        except tarfile.ReadError:
            print("⚠️  Detected corrupted PaddleOCR archive, removing directory")
            try:
                shutil.rmtree(rec_dir)
            except Exception as exc:
                print(f"!! failed to remove corrupted directory: {exc}")

# clean up any existing bad files before initialization
